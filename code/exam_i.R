...
# Assign to: `mtcars_light_cyl4`filer
...
mtcars_mpg_by_gear<-df_mtcars %>% group_by(gear) %>% summarize(min_mpg=min(mpg), max_mpg=max(mpg))
...
iris %>% ggplot(aes(x=Petal.Width))+geom_histogram(binwidth=0.5)
...
g_bplot2<-PlantGrowth %>% ggplot(aes(x=group,y=weight,fill=group))+geom_boxplot()+geom_point()
##same outout
g_bplot2<-g_bplot1+geom_point()
